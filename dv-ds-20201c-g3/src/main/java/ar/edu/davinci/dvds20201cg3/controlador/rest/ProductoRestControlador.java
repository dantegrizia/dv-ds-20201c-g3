...
import ar.edu.davinci.dvds20201cg3.servicio.ProductoServicio;
...
public class ProductoRestControlador extends MyRestApp {
	...
	public Page<ProductoResponse> getProductos(Pageable pageable) {
		...
		try {
			productosResponse = productosPage.map(cliente -> mapper.map(cliente, ProductoResponse.class));
		} catch (Exception e) {
			LOGGER.error(e.getMessage());
			e.printStackTrace();
		}
	...
	public ResponseEntity<ProductoResponse> updateProducto(@PathVariable("id") long id,
		...
		productoModificar = productoServicio.buscarPorId(id);

		if (Objects.nonNull(productoModificar)) {
			productoModificar.setName(productoNuevo.getName());
			productoModificar.setPrice(productoNuevo.getPrice());
			productoModificar = productoServicio.grabarProducto(productoModificar);
		} else {
		...
		// Convertir Producto en ProductoResponse
		try {
			productoResponse = mapper.map(productoModificar, ProductoResponse.class);
		} catch (Exception e) {
			LOGGER.error(e.getMessage());
			e.printStackTrace();
		}
	...
	public @ResponseBody Page<Producto> getProductoPaginado() {
		Pageable pageable = PageRequest.of(0, 10);
		return productoServicio.listar(pageable);
	}

	@GetMapping(path = "/productos/buscar")
	public @ResponseBody Iterable<Producto> buscarProducto(@RequestParam String nombre) {
